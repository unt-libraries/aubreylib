import urlparse, httplib, os, re, urllib, urllib2, string
from pypairtree.pairtree import pair_tree_creator, get_pair_path
...
def get_file_system(meta_id, file_path, location_tuple):
    ...
    for file_system in location_tuple:
        ...
        if re.compile(r'^file://').search(file_system, 0) != None:
            ...
            #if the file name starts with file://, change it to start with just a /
            ...
            try:
                #if the file name starts with file:// or /, change it to start with no beginning slashes
                if file_path.startswith('file://'):
